...
$auth =auth()->user()->w_api_token;

        // $profileId = 'aedd0dc2-8453';
        $profileId = auth()->user()->w_api_profile_id;
...
if(auth()->user()->enable_whatsapp_notifications){
    if(auth()->user()->enable_employee_notifications){
        $response = WhatsAppService::send_with_wapi($auth, $profileId, $phone, $message);
    }
    if(auth()->user()->work_group!=null && auth()->user()->work_group!=''){
        $genral_group = auth()->user()->work_group;
        $response = WhatsAppService::send_with_wapi($auth, $profileId, $genral_group, $message);
    }

        if($data['send_to_group'] &&auth()->user()->enable_group_notifications){
